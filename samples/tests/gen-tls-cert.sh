...
# ============================================================================
#  gen-tls-cert.sh
#  Generate self-signed test certs for HorseNghttp2TestServer TLS + mTLS modes.
#
...
openssl req -newkey rsa:2048 -nodes \
    ...
    -subj "/CN=nghttp2-test-client/O=HorseNghttp2Test/OU=Client" \
    2>&1 | grep -v "^\.\+$" || true
...
echo
echo "Server modes:"
echo "  HorseNghttp2TestServer.exe             (h2c on 9010)"
echo "  HorseNghttp2TestServer.exe tls         (h2 over TLS on 9443)"
echo "  HorseNghttp2TestServer.exe mtls        (h2 over TLS + client cert required)"
echo
echo "Client test — curl:"
echo "  curl --http2 --insecure https://127.0.0.1:9443/ping"
echo "  curl --http2 --insecure --cert ${CERT_DIR}/client-cert.pem --key ${CERT_DIR}/client-key.pem https://127.0.0.1:9443/ping"
