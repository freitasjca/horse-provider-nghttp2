...
The gRPC layer is **not** in this repository. `Nghttp2.Grpc.{Attributes,Registry,Dispatcher,StreamReader,StreamWriter}` live in `Delphi-nghttp2` (≥ 1.5.0), because they only ever took an `INghttp2Stream` — no Horse type appeared in any of them. This provider's entire involvement is two guarded lines in `Horse.Provider.Nghttp2`: `TGrpcDispatcher.TryDispatch` ahead of the pool acquire, and `TGrpcRegistry.IsInboundStreaming` on the transport's inbound hook. Both compile out under `HORSE_NGHTTP2_NO_GRPC`.
